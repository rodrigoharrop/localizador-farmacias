...
const REDES_INFO = {
  'PAGUE MENOS': { logo: 'Logotipo_da_Pague_Menos.svg.webp', iconMapa: 'icone Pague Menos.png', color: '#003DA5' },
  'DROGASIL': { logo: 'Logotipo_da_Drogasil_(2024).svg.webp', iconMapa: 'icone Drogasil.jpg', color: '#E63946' },
  'EXTRAFARMA': { logo: 'Extrafarma.png', iconMapa: 'icone ExtraFarma.jpeg', color: '#6B46C1' },
  'OUTRAS': { logo: null, iconMapa: null, color: '#0F7D3F' }
};
...
function obterLocalizacao() {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        localizacaoAtual = { lat: position.coords.latitude, lng: position.coords.longitude };
...
function criarIconeRede(rede) {
  ...
  if (redeInfo.iconMapa) {
    return L.icon({ iconUrl: redeInfo.iconMapa, iconSize: [40, 40], iconAnchor: [20, 40], popupAnchor: [0, -40] });
  }
  return L.divIcon({
    html: `<div style="display:flex;align-items:center;justify-content:center;width:40px;height:40px;background:${redeInfo.color};border-radius:50%;color:white;font-size:20px;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3);font-weight:bold;">💊</div>`,
...
function criarIconeLocal() {
  return L.divIcon({
    ...
    iconSize: [36, 36], className: 'custom-marker-local'
  });
}
...
function inicializarMapa(lat, lng) {
  ...
  if (localizacaoAtual) {
    L.marker([lat, lng], { icon: criarIconeLocal() }).addTo(mapa).bindPopup('<b>📍 Sua localização</b>').openPopup();
  }
  ...
  farmaciasVisiveis.forEach(farmacia => {
    const marker = L.marker([farmacia.lat, farmacia.lng], { icon: criarIconeRede(farmacia.rede) }).addTo(mapa);
      ...
      <div style="width: 240px;">
        ...
        <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #eee;">
          ...
          <div style="margin-top: 8px; display: flex; gap: 6px;">
            ${localizacaoAtual ? `<button onclick="mostrarRota(${localizacaoAtual.lat}, ${localizacaoAtual.lng}, ${farmacia.lat}, ${farmacia.lng})" style="flex: 1; padding: 6px; background: #0F7D3F; color: white; border: none; border-radius: 4px; font-size: 12px; font-weight: 600; cursor: pointer;">🗺️ Ver Rota</button>` : ''}
            <button onclick="abrirGoogleMaps('${farmacia.nome.replace(/'/g, "\\'")}', '${farmacia.endereco.replace(/'/g, "\\'")}', '${farmacia.cidade}')" style="flex: 1; padding: 6px; background: #666; color: white; border: none; border-radius: 4px; font-size: 12px; font-weight: 600; cursor: pointer;">📍 Google Maps</button>
          </div>
        </div>
      </div>
    `);
...
function mostrarRota(latOrigem, lngOrigem, latDestino, lngDestino) {
  if (window.rotaAtual) mapa.removeLayer(window.rotaAtual);
  
  const latlngs = [[latOrigem, lngOrigem], [latDestino, lngDestino]];
  window.rotaAtual = L.polyline(latlngs, { color: '#0066FF', weight: 4, opacity: 0.8, dashArray: '0' }).addTo(mapa);
  
  const group = new L.featureGroup([L.marker([latOrigem, lngOrigem]), L.marker([latDestino, lngDestino])]);
...
function mostrarRotaDoCard(latDestino, lngDestino) {
  if (localizacaoAtual) {
    ...
    const latlngs = [[localizacaoAtual.lat, localizacaoAtual.lng], [latDestino, lngDestino]];
    window.rotaAtual = L.polyline(latlngs, { color: '#0066FF', weight: 4, opacity: 0.8, dashArray: '0' }).addTo(mapa);
    
    const group = new L.featureGroup([L.marker([localizacaoAtual.lat, localizacaoAtual.lng]), L.marker([latDestino, lngDestino])]);
    mapa.fitBounds(group.getBounds(), { padding: [50, 50] });
  }
}
...
function exibirFarmaciasGroupadas() {
  ...
  FARMACIAS.forEach(f => {
    ...
    if (!farmaciasGroupadas[f.rede]) farmaciasGroupadas[f.rede] = [];
    let farmacia = { ...f };
    if (localizacaoAtual) farmacia.distancia = calcularDistancia(localizacaoAtual.lat, localizacaoAtual.lng, f.lat, f.lng);
    farmaciasGroupadas[f.rede].push(farmacia);
  });

  Object.keys(farmaciasGroupadas).forEach(rede => {
    if (localizacaoAtual) farmaciasGroupadas[rede].sort((a, b) => a.distancia - b.distancia);
  });

  let html = '<div class="redes-container">';
  const ordemRedes = ['PAGUE MENOS', 'DROGASIL', 'EXTRAFARMA', 'OUTRAS'];
  Object.keys(farmaciasGroupadas).sort((a, b) => ordemRedes.indexOf(a) - ordemRedes.indexOf(b)).forEach(rede => {
...
function filtrarRede(rede) {
  ...
  inicializarMapa(localizacaoAtual ? localizacaoAtual.lat : FORTALEZA_CENTER.lat, localizacaoAtual ? localizacaoAtual.lng : FORTALEZA_CENTER.lng);
  exibirFarmaciasGroupadas();
}
